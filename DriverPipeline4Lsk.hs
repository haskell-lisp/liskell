...
#warning FIXME: preprocess liskell files?
  if isLiskellSrcFilename filename then	return (hsc_dflags hsc_env, filename)
  else
 ...
 = do
  ...
  -- imports a _stub.h file that we created here.
   ...
   let getStubLinkable False = return []
       ...
       handleBatch HML.HscNoRecomp
           = ASSERT (isJust maybe_old_linkable)
             return maybe_old_linkable

       handleBatch (HML.HscRecomp hasStub)
       ...
       handleInterpreted HML.InteractiveNoRecomp
           = ASSERT (isJust maybe_old_linkable)
             return maybe_old_linkable
       handleInterpreted (HML.InteractiveRecomp hasStub comp_bc modBreaks)
           = do stub_unlinked <- getStubLinkable hasStub
   ...
   case hsc_lang of
      HscInterpreted
        | isHsBoot src_flavour -> 
                runCompiler HML.hscCompileNothing handleBatch
        | otherwise -> 
                runCompiler HML.hscCompileInteractive handleInterpreted
      HscNothing -> 
                runCompiler HML.hscCompileNothing handleBatch 
      _other -> 
                runCompiler HML.hscCompileBatch handleBatch
