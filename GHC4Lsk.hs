...
import TcRnMonad        ( initIfaceCheck, getLIE, initTcPrintErrors, failIfErrsM )
import Packages
import NameSet
import RdrName
...
guessTarget str Nothing
   ...
   = do exists <- liftIO $ doesFileExist hs_file
	...
	exists <- liftIO $ doesFileExist lsk_file
	if exists
	   then return (target (TargetFile lsk_file Nothing))
	   else do
...
hscInteractive' hsc_env mod_summary (iface, details, cgguts)
    = do
...
--             <- outputForeignStubs dflags this_mod location foreign_stubs
	 clock <- getClockTime 
	 return (LM clock this_mod [BCOs comp_bc emptyModBreaks])
...
loadModule tcm = do
   ...
   modguts <- deSugarModule hsc_env ms tcg
   modguts' <- liftIO $ core2core hsc_env modguts
   (cgguts, _moddetails) <- liftIO $ tidyProgram hsc_env modguts'
   linkable <- liftIO $ hscInteractive' hsc_env ms (iface, details, cgguts)
...
downsweep hsc_env old_summaries excl_mods allow_dup_roots
   = do -- catch error messages and return them
	...
	getRootSummary (Target (TargetFile file mb_phase) obj_allowed maybe_buf)
	   = do
--	        liftIO $ putStrLn "getRootSummary for TargetFile"
	...
	getRootSummary (Target (TargetModule modl) obj_allowed maybe_buf)
 	   = do 
--  	        liftIO $ putStrLn "getRootSummary for TargetModule"
                maybe_summary <- summariseModule hsc_env old_summary_map False 
...
summariseFile hsc_env old_summaries file mb_phase obj_allowed maybe_buf
  ...
  where
	...
	(dflags', hspp_fn, buf)
        ...
        (srcimps,the_imps, L _ mod_name) <- liftIO $ LskFileHandler.getImportsLsk dflags' buf hspp_fn file
...
summariseModule hsc_env old_summary_map is_boot (L loc wanted_mod) 
  ...
  where
	...
	found <- liftIO $ FL.findImportedModule hsc_env wanted_mod Nothing
	case found of
	     Found location mod 
	...
	(dflags', hspp_fn, buf) <- preprocessFile hsc_env src_fn Nothing maybe_buf
        (srcimps, the_imps, L mod_loc mod_name) <- liftIO $ getImportsLsk dflags' buf hspp_fn src_fn
...
compileHsExpr hsc_env rdr_expr get_expr_type = do
    let ictxt = hsc_IC hsc_env
    (msg,maybe_tc_expr) <- liftIO $ 
		 initTcPrintErrors hsc_env iNTERACTIVE $ 
		 setInteractiveContext' hsc_env ictxt $ do {
			  (rn_expr, fvs) <- rnLExpr rdr_expr ;
			  failIfErrsM ;
			  expr_type <- get_expr_type;
			  (tc_expr, lie) <- getLIE $ tcMonoExpr rn_expr expr_type;
			  const_binds <- tcSimplifyTop lie;
			  tc_expr' <- zonkTopLExpr (mkHsDictLet const_binds tc_expr);
			  return tc_expr';
			}
    liftIO $ printErrorsAndWarnings (hsc_dflags hsc_env) msg 
    case maybe_tc_expr of
      Nothing -> throw $ mkSrcErr (unitBag $ mkPlainErrMsg noSrcSpan (text "Type checking failed"))
      (Just tc_expr) -> do
        let rdr_env  = ic_rn_gbl_env ictxt
	    type_env = mkTypeEnv (map AnId (ic_tmp_ids ictxt))
	ds_expr <- ioMsgMaybe $
			deSugarExpr hsc_env iNTERACTIVE rdr_env type_env tc_expr
	let src_span = srcLocSpan interactiveSrcLoc
--	liftIO $ Linker.initDynLinker (hsc_dflags hsc_env)
--	liftIO $ Linker.showLinkerState
	hval <- liftIO $ HscMain.compileExpr hsc_env src_span ds_expr
--	liftIO $ Linker.showLinkerState
	return $ Just hval
  where
